...
void setup() {
  ...
  ShapeSize=int(height/MapSize);                         //Initialize Variables
  SnakeX=new int[1024];
  SnakeY=new int[1024];
...
void draw() {
  background(127, 127, 127);                             //Start Rendering
  fill(0, 0, 0);
  noStroke();
  translate((width-height)/2, (height-MapSize*ShapeSize)/2);
...
void NewO() {                                            //Generate New Circle
  boolean OUse=true;                                     //Circle Position Avaliability (True As Invalid, False As Valid)
  int OUseX=0;
  int OUseY=0;

  while (OUse==true) {
    OUseX=int(random(0, 31));                            //Generate New Circle Position
    OUseY=int(random(0, 31));
    OUse=false;

    for (int CUse=0; CUse<Length-1; CUse++) {            //Check Avaliability
